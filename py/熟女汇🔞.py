...
class Spider(BaseSpider):
    ...
    PAGE_SIZE = 30
    CATEGORIES = (
        ('最新视频', 'newest'('亚洲情色', '374'), ('主播自拍', '375'),
		('国产偷拍', '376'),('无码系列', '377'), ('欧美性爱', '378'),
		('熟女专区', '379'),('强奸系列', '380'), ('巨乳系列', '381'),
		('中文大全', '382'),('制服学生', '383'), ('女同蕾丝', '384'),
		('卡通动画', '385'),('视频伦理', '386'), ('少女裸体', '387'),
		('重口色情', '388'),('人兽性交', '389'), ('福利姬', '473'),
		('精品推荐', '224'),('国产色情', '225'), ('主播直播', '227'),
		('亚洲无码', '229'),('亚洲有码', '231'), ('中文有码', '233'),
		('巨乳美乳', '235'),('人妻系列', '237'), ('强奸精品', '239'),
		('欧美精品', '241'),('萝莉少女', '243'), ('伦理三级', '245'),
		('自拍偷拍', '249'),('制服丝袜', '251'), ('口交颜射', '253'),
		('日本精品', '255'),('Cosplay', '257'), ('素人自拍', '259'),
		('台湾辣妹', '261'),('韩国御姐', '263'), ('唯美港姐', '265'),
		('东南亚AV', '267'),('欺辱凌辱', '269'), ('剧情介绍', '271'),
		('多人多P', '273'),('91探花', '275'), ('网红流出', '276'),
		('野外露出', '277'),('古装扮演', '278'), ('女优系列', '279'), 
		('可爱学生', '280'),('风情旗袍', '281'), ('兽耳系列', '282'), 
		('瑜伽裤', '283'),('闷骚护士', '284'), ('过膝袜', '285'), 
		('网曝门', '286'),('传媒出品', '287'), ('女同性恋', '288'), 
		('男同性恋', '289'),('恋腿狂魔', '290'), 
    )
    ...
    def categoryContent(self, tid, pg, filter=False, extend=None):
        ...
        path = '/show/newest.html' if str(tid) == 'newest' else '/type/id/%s.html' % tid
        if page > 1:
            path = path.replace('.html', '-p%s.html' % page)
        ...
        return {'list': videos, 'page': page, 'pagecount': 9999 if videos else page,
    ...
    def searchContent(self, key, quick=False, pg='1'):
        ...
        path = '/lookup/%s.html' % quote(keyword, safe='')
        if page > 1:
            path = '/lookup/%s-%s.html' % (quote(keyword, safe=''), page)
        response = self._request(path)
        videos = self._cards(response.text, response.url) if response else []
        return {'list': videos, 'page': page, 'pagecount': 9999 if videos else page,
                'limit': len(videos) or self.PAGE_SIZE, 'total': 0}
    ...
    def detailContent(self, ids):
        ...
        if not play_urls:
            play_urls = re.findall(r'https?://[^"\'\s<>]+(?:m3u8|mp4|m3u|mpd)[^"\'\s<>]*', play_html, re.I)
